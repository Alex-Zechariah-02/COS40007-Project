...
tab_target, tab_row, tab_figures, tab_evidence = st.tabs(["Target", "Row unit", "Figures", "Evidence tables"])

with tab_target:
    why_not("Why this is regression", WHY_REGRESSION)
    for fname, title in [("main_supervised_target_panel.csv", "Main supervised target panel"), ("malaysia_benchmark.csv", "Malaysia benchmark rows")]:
        df = load_table(fname)
        if not df.empty:
            show_table(df, title, max_rows=15)

with tab_row:
    both_df = load_table("both_only_target_panel.csv")
    if not both_df.empty:
        show_table(both_df, "Both-only aggregate panel", max_rows=15)
