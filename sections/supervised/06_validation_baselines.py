...
from src.ui_components import page_header, render_page_intro, render_page_discussion, show_table, show_table_with_guide, metric_cards, guided_figure, metric_direction_guide, next_steps
...
tab_split, tab_baselines, tab_metrics, tab_figures = st.tabs(["Split design", "Baselines", "Metric guide", "Figures"])

with tab_split:
    if not split.empty:
        show_table_with_guide(split, "split_integrity_check.csv", "Split integrity check", max_rows=20)
    folds = load_table("chronological_validation_folds.csv")
    if not folds.empty:
        show_table(folds, "Chronological validation folds", max_rows=20)

with tab_baselines:
    for fname, title in [("baseline_metrics.csv", "Core baseline metrics"), ("additional_rule_baseline_metrics.csv", "Additional rule baseline metrics"), ("all_baseline_metrics_pre_svr.csv", "All baseline metrics before SVR")]:
        df = load_table(fname)
        if df.empty:
            df = load_check(fname)
        if not df.empty:
