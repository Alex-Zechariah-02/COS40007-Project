...
tab_tables, tab_groups, tab_figures, tab_limits = st.tabs(["Importance tables", "Feature groups", "Figures", "Interpretation limits"])

with tab_tables:
    imp = load_table("permutation_importance.csv")
    if not imp.empty:
        show_table_with_guide(imp, "permutation_importance.csv", "Permutation importance", max_rows=20)
    df = load_table("permutation_importance_with_groups.csv")
    if not df.empty:
        show_table_with_guide(df, "permutation_importance_with_groups.csv", "Permutation importance with feature groups", max_rows=20)
...
with tab_limits:
    st.warning("Feature influence is diagnostic model-behaviour evidence only. It must not be interpreted as causal impact on school completion rates.")
