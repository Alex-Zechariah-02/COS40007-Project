...
tab_selected, tab_validation, tab_tuning, tab_figures = st.tabs(["Selected candidate", "Validation", "Tuning tables", "Figures"])
...
with tab_validation:
    for fname, title in [("svr_validation_summary.csv", "SVR validation summary"), ("svr_fold_metric_variability.csv", "Fold metric variability"), ("ridge_benchmark_metrics.csv", "Ridge benchmark metrics"), ("final_svr_training_audit.csv", "Final SVR training audit")]:
        df = load_table(fname)
        if df.empty:
            df = load_check(fname)
        if not df.empty:
...
with tab_tuning:
    for fname, title in [("svr_validation_results.csv", "SVR validation results"), ("svr_tuning_audit.csv", "SVR tuning audit")]:
        df = load_table(fname)
        if df.empty:
            df = load_check(fname)
        if not df.empty:
            show_table(df, title, max_rows=25)
