...
with tab_metrics:
    ...
    for fname, title in [("residual_summary.csv", "Residual summary"), ("svr_metric_card_summary.csv", "Practical diagnostic metrics")]:
        df = load_table(fname)
        if not df.empty:
...
with tab_grouped:
    ...
    for fname, title in [("error_by_state.csv", "Error by state"), ("error_by_stage.csv", "Error by stage"), ("error_by_sex.csv", "Error by sex"), ("error_by_state_stage_sex.csv", "Detailed state-stage-sex errors")]:
        df = load_table(fname)
        if not df.empty:
            show_table_with_guide(df, fname, title, max_rows=20)

with tab_worst:
    df = load_table("worst_prediction_rows.csv")
    if not df.empty:
        show_table_with_guide(df, "worst_prediction_rows.csv", "Worst prediction rows", max_rows=20)
