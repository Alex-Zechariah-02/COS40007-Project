...
from src.ui_components import page_header, render_page_intro, render_page_discussion, show_table_with_guide, metric_cards, abbreviation_note, guided_figure, next_steps
...
if not wins.empty:
    cards = {str(row["winner_by_row"]).upper(): f"{int(row['row_count'])} rows" for _, row in wins.iterrows()}
...
tab_wins, tab_rows, tab_disagree, tab_figures = st.tabs(["Winner summary", "Row-level table", "Largest disagreements", "Figures"])
...
with tab_figures:
    fig = "row_level_winner_summary.png"
    e = explain(fig)
    guided_figure(comparison_figure_path(fig), "Row-level winner summary", e["purpose"], e["how"], e["better"], e["takeaway"], e.get("caveat"))
